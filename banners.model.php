...
class BannersModel extends Model
{
    ...
    public function bannersData(array $aFilter = array())
    {
        if (($aData = $this->cache()->get($this->cacheKey)) === false) {
            ...
            foreach ($aData as &$v) {
                ...
                # данные о специализации
                $v['specialization'] = (!empty($v['specialization_id']) ? explode(',', $v['specialization_id']) : array());
                # данные о категориях
    ...
    public function bannersListing(array $aFilter = array())
    {
        ...
        if (isset($aFilter['region'])) {
            $aRegion = Geo::regionData($aFilter['region']);
            if (!empty($aRegion['id'])) {
                switch ($aRegion['numlevel']) {
                    case Geo::lvlCity: $aFilter['region_id'] = $aRegion['id']; break;
                    case Geo::lvlRegion:
                        $region = true;
                        $aFilter[':region'] = array('( R.pid = :region OR B.region_id = :region)', ':region' => $aRegion['id']);
                        break;
                    case Geo::lvlCountry:
                        $region = true;
                        $aFilter[':region'] = array('( R.country = :region OR B.region_id = :region)', ':region' => $aRegion['id']);
                        break;
                }
            }
        ...
        return $this->db->select('SELECT B.*, SUM(S.shows) as shows, SUM(S.clicks) as clicks
                               FROM ' . TABLE_BANNERS . ' B
                                 ...
                                 '.($region ? ' LEFT JOIN '.TABLE_REGIONS.' R ON B.region_id = R.id ' : '').'
                               ' . $aFilter['where'] . '
                               GROUP BY B.id',
            $aFilter['bind']
        );
